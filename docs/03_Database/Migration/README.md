...
Status: P1 Migrations remain unapplied to Production. Dedicated QA Attempt 1 applied 1–19 and failed D; R1 corrects D locally pending Closure Commit and a separately authorized clean QA retry.
...
The migrations are additive and ordered. They create no cloud resources and must be applied through the normal Supabase migration workflow only after the target environment and bootstrap operator are explicitly approved. Dedicated QA Attempt 1 is preserved as a failed 19/20 bootstrap with emergency fail-closed; it is not an upgrade baseline. Migration D's R1 correction explicitly normalizes PostgreSQL `PUBLIC` and Supabase Hosted direct application-role function grants before asserting the legacy deny matrix. Production remains unapplied. Any new hosted proof requires a clean QA environment and separate Product Owner authorization.
