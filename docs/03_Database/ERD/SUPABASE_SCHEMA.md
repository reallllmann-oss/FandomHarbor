...
| `private.identity_access_request_ledger` (P1 proposed) | request UUID, actor/target/operation, SHA-256 payload fingerprint, closed result snapshot, optional unique Audit reference | request PK, actor/date, target/date |
...
Admin P1-01 proposes one new private table only: `identity_access_request_ledger`. It is not a role or Membership fact and is never exposed through the Data API. It retains the global request UUID, actor/target/operation, normalized payload fingerprint and original `saved/unchanged/conflict` result so every result is replayable without a duplicate business change or Audit. Under ADR-022 Option 3, its current operation constraint contains only Author Grant, Author Revoke and ordinary-account Membership state change; it cannot record or authorize an elevated operation. The authoritative proposal, RLS/grant boundary and indefinite retention rule are in [`P1_01_DATA_PERMISSION_REAUTH_DESIGN.md`](../../15_Sprint/Admin_P1/P1_01_DATA_PERMISSION_REAUTH_DESIGN.md). No P1-01 Migration exists.
