...
- 2026-07-25：完成 V1 Public Policy Owner Decision Application。27 / 27 决策均为 `OWNER DECIDED`；`V1-PUBLIC-POLICY-V1.0.md` 成为唯一当前规范源，Draft 与 2026-07-16 历史政策保留并标记 superseded。V1.0 使用运营主体“Fandom Harbor，由刘祯莹个人运营”、邮箱 `fandomharbor@163.com`、批准日期 2026-07-25、正式网站发布生效；同步 Guest / Reader / Membership、30 天导出 / 删除目标、最小身份核验、活动数据与备份轮换、7 日 + 4 周内部备份目标、Auth / Storage 排除、18+、未成年人、真实人物、禁止内容、人工侵权 / 下架、非完整 DMCA、中华人民共和国法律与争议规则。三个政策页面已同步；Footer、注册提示、`/legal` redirect、SEO / sitemap 保持。独立法律审阅 `NOT COMPLETED`，Owner 接受有限 Reader-only Beta 风险。`pnpm validate`、本地 Production build 浏览器检查、diff check 与 30 文件 Secret scan 均通过，Product P0 / P1=0 / 0；XML 浏览器直开受客户端限制并如实标记手工验证。`PDR-01 = OWNER DECISION COMPLETE / DEPLOYMENT AND SMOKE PENDING`，PDR-02 保持关闭，Deployment Authorized=`NO`，Final Acceptance=`PENDING`。下一项为 Production Policy Release Candidate Commit and Deployment Preparation。

- 2026-07-25：完成 V1 Production Release Status Reconciliation and Owner Decision Pack，结论 `PASS WITH OWNER DECISION REQUIRED`。新增 27 项可填写政策决策包与统一 Production Release Closure；建立 17 项 Gate Matrix，并逐文件分类 Mission 开始时 15 modified / 12 untracked，加上本 Mission 两份新文档共 29 项。Policy APP 条件性 RC 为 11 个 Web / UI 文件；3 个 Admin 文件必须排除，历史 `V1-PUBLIC-POLICY.md` 因声称已批准而归入 Needs Review，不得作为当前规范源。当前候选源为 Draft；产品对照确认 Guest 可发现 Published 内容但 Work Detail / Chapter 需登录 + active Membership，Reader 无 Studio，Author 可创建 / 保存 / 发布，邀请码仍为注册必需；需澄清公开作者信息、浏览器 localStorage 和独立 Articles。Production domain / Web / Supabase / Reader Smoke / Author loop / PDR-02 均有 PASS 证据，Policy Approval / Deployment / Production Policy Smoke / Owner Final Acceptance 未完成。`PDR-01 = OWNER REVIEW REQUIRED`，`PDR-02 = CLOSED FOR CURRENT RELEASE`，Admin Preview 独立 BLOCKED 但不直接阻挡 Reader-only Beta，Production Deployment Authorized=`NO`，Release Approved=`NO`。下一项 Mission=`Public Policy Owner Decision Application`。

- 2026-07-25：完成 V1 Current Production Fresh Backup and PDR-02 Closure，结论 `PASS`。Product Owner 在本机终端通过仓库外 `0700` 一次性脚本隐藏输入密码；Codex 未接收或记录 Secret。linked `fandom-harbor` Production 的 `public,private` Schema dump 与 Data-only + COPY dump 创建于 2026-07-25 22:38:50 +08:00，退出码均为 0；Schema 93,346 bytes / SHA-256 `252e605784de4d75f72c7f8afbe4cf2b9d5128ba1319856591a2999e727ac3e5`，Data 232,260 bytes / SHA-256 `f08bd95ba0ecfae5d7147e85fab5b70be320415b2860a1b0efc51755b468d534`，Manifest 1,949 bytes / SHA-256 `42f2f0800ef9d701c3be2b69f59c492571783e787c127365a787f6f2f05dd861`。目录 / 文件权限 `0700` / `0600`，15 / 15 表定义、COPY 段 / 结束标记、必要应用表、错误 / NUL / 截断、Git 边界与历史检查均 PASS；历史备份保持 `VALID BUT STALE`。Owner 确认 Plan=Free、数据库约 28 MB、MAU=25、平台备份=0、PITR=false、Storage Bucket / 对象=0。Auth 管理数据与 Storage 不在逻辑备份范围，恢复演练 `NOT RUN`。`PDR-02 = CLOSED FOR CURRENT RELEASE`；`PDR-01 = OWNER REVIEW REQUIRED`，Production Deployment Authorized=`NO`。

- 2026-07-25：完成 V1 Supabase Production Backup Evidence and Recovery Readiness Audit，结论 `PASS WITH OWNER VERIFICATION`。2026-07-16 备份的 Schema SQL（93,346 bytes，SHA-256 `252e605784de4d75f72c7f8afbe4cf2b9d5128ba1319856591a2999e727ac3e5`）与 Data SQL（204,387 bytes，SHA-256 `4317c21e8e63aedcc1f824d3b5da6df777026850873549d685474ce47593bbc3`）在仓库外真实存在，权限、类型、15 / 15 表结构、COPY 终止、错误标记与 Git 边界复验通过；实物为 `VALID`，对当前 Production 新鲜度为 `STALE`。只读 API 当前确认 Project ACTIVE_HEALTHY、region `ap-southeast-1`、平台备份记录=0、PITR=false；当前套餐须 Owner 在 Dashboard 核对。新鲜导出因临时只读角色连接超时且后续需要显式数据库密码而安全停止，未生成新备份、未输出 Secret、未触碰 Production。新增备份证据与恢复 Runbook；恢复演练 `NOT RUN`。`PDR-02 = OWNER VERIFICATION REQUIRED / NOT CLOSED FOR CURRENT RELEASE`，须完成当前套餐 / Dashboard 核对与新鲜仓库外备份后重新评审；`PDR-01 = OWNER REVIEW REQUIRED` 保持不变，Production Deployment 仍未授权。

- 2026-07-25：完成 V1 Public Policy Web Pages Implementation。以 `docs/19_Release/V1-PUBLIC-POLICY-DRAFT.md` 为唯一主要内容来源，新增 Guest 与所有登录角色可访问的 `/privacy`、`/terms`、`/content-policy`，复用现有 ReaderLayout / 主题 / token；Reader Footer 和注册页均增加 Privacy / Terms / Content Policy 入口，旧 `/legal` 永久重定向到 `/terms`，sitemap 更新为三个规范路由。1280px / 390px、Light / Dark、Guest 直达 / 刷新、Footer / 注册点击、metadata、无横向溢出与 Console Error=0 均 PASS；Lint、TypeScript、自动化测试、Build、diff check 与 Secret 扫描通过。来源仍为 Draft、待 Product Owner 批准、生效状态未确认、法律审阅未完成，并含备份期限、处理地区、争议与其他法律条款待决项，因此结论为 `PASS WITH OWNER REVIEW`，PDR-01 不得声称完全 Release Approved。PDR-02 状态保持进入 Mission 前记录不变；未执行数据库备份、数据库 / Auth / RLS / RPC / Migration、权限操作、提交、推送或 Production Deployment。
